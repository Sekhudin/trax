...
func TestGenerate_Success(t *testing.T) {
	...
	t.Run("default_deps_structure", func(t *testing.T) {
		...
		if d.Docs.Root.Use != "generate" || d.Docs.Config.Use != "config" || d.Docs.Routes.Use != "routes" {
			t.Fatal("fail")
		}

		if len(d.Docs.Root.Aliases) != 1 && d.Docs.Root.Aliases[0] != "g" {
			t.Fatal("fail")
		}
	})
	...
	t.Run("new_default_instance", func(t *testing.T) {
		...
		cmd := New(ctx)
		if cmd.Use != "generate" || len(cmd.Commands()) != 2 {
			t.Fatal("fail")
		}
	})
}
...
func TestGenerate_Fallback(t *testing.T) {
